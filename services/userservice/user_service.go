...
func NewUserService(datastore datastore.Datastore, pubsub pubsub.Pubsub) user2.UserService {
	return &userService{
		datastore: datastore,
		pubsub:    pubsub,
	}
...
func (s *userService) Put(ctx context.Context, user user2.User) error {
	...
	err := s.datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		if !exists {
			err := s.pubsub.Publish(ctx, user2.UserTopicName, user2.UserCreatedEvent{
				State: user,
			})
			if err != nil {
			...
			err := s.pubsub.Publish(ctx, user2.UserTopicName, user2.UserModifiedEvent{
...
func (s *userService) Remove(ctx context.Context, userUID string) error {
	err := s.datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		if exists {
			...
			err = s.pubsub.Publish(ctx, user2.UserTopicName, user2.UserRemovedEvent{
...
func (s *userService) Query(ctx context.Context, filterFunc user2.UserFilterFunc) ([]user2.User, error) {
	users := []user2.User{}
	var err error
